...
export const renderSlopedTileRoof = (
  scene: THREE.Scene,
  px: number,
  pz: number,
  width: number,
  ...
  // Fascia & Gutters
  const fasciaGeo = new THREE.BoxGeometry(rl, fascia_h, fascia_t);
  const ff = new THREE.Mesh(fasciaGeo, fasciaStyle);
  ff.position.set(0, rz_eave - fascia_h / 2, ry_eave_f - fascia_t / 2);
  group.add(ff);
  
  const fb = new THREE.Mesh(fasciaGeo, fasciaStyle);
  fb.position.set(0, rz_eave - fascia_h / 2, ry_eave_b + fascia_t / 2);
  group.add(fb);

  const gutterGeo = new THREE.BoxGeometry(rl, gutter_h, gutter_w);
  const gf = new THREE.Mesh(gutterGeo, gutterStyle);
  gf.position.set(0, rz_eave - fascia_h - gutter_h / 2, ry_eave_f - fascia_t - gutter_w / 2);
  group.add(gf);

  const gb = new THREE.Mesh(gutterGeo, gutterStyle);
  gb.position.set(0, rz_eave - fascia_h - gutter_h / 2, ry_eave_b + fascia_t + gutter_w / 2);
  group.add(gb);
