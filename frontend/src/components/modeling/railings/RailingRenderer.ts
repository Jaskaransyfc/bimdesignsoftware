...
const renderModernMetalRailing = (
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    const topRail = new THREE.Mesh(
      new THREE.BoxGeometry(dist, 0.04, frameW),
      mat,
    );
    ...
    const botRail = new THREE.Mesh(
      new THREE.BoxGeometry(dist, 0.03, frameW),
      mat,
    );
    ...
    for (let j = 1; j <= nBars; j++) {
      const bar = new THREE.Mesh(
        new THREE.BoxGeometry(barW, height - 0.15, barW),
        mat,
      );
...
const renderGlassPanelRailing = (
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    const glass = new THREE.Mesh(
      new THREE.BoxGeometry(dist, height - 0.1, glassThk),
      glassMat,
    );
    ...
    const handrail = new THREE.Mesh(
      new THREE.CylinderGeometry(railR, railR, dist, 12),
      metalMat,
    );
...
const makeBranchRecursive = (
  ...
  for (let i = 0; i < nChildren; i++) {
    const nextDir = dir
      ...
      .applyAxisAngle(new THREE.Vector3(1, 0, 0), (Math.random() - 0.5) * 0.5);
    makeBranchRecursive(
      parent,
      end,
      nextDir,
      length * (0.6 + Math.random() * 0.2),
      depth - 1,
      material,
    );
  }
